...
let rec g env (pos, ebody) = (* 定数畳み込みルーチン本体 (caml2html: constfold_g) *)
  match ebody with
  | Var(x) when memi x env -> pos, Int(findi x env)
  (* | Var(x) when memf x env -> pos, Float(findf x env) *)
  (* | Var(x) when memt x env -> Tuple(findt x env) *)
  | Neg(x) when memi x env -> pos, Int(-(findi x env))
  | Add(x, y) when memi x env && memi y env -> pos, Int(findi x env + findi y env) (* 足し算のケース (caml2html: constfold_add) *)
  | Sub(x, y) when memi x env && memi y env -> pos, Int(findi x env - findi y env)
  | FNeg(x) when memf x env -> pos, Float(-.(findf x env))
  | FAdd(x, y) when memf x env && memf y env -> pos, Float(findf x env +. findf y env)
  | FSub(x, y) when memf x env && memf y env -> pos, Float(findf x env -. findf y env)
  | FMul(x, y) when memf x env && memf y env -> pos, Float(findf x env *. findf y env)
  ...
  | Let((x, t), e1, e2) -> (* letのケース (caml2html: constfold_let) *)
      let e1' = g env e1 in
      let (_, e1'body) = e1' in
      let e2' = g (M.add x e1'body env) e2 in
      pos, Let((x, t), e1', e2')
  | LetRec({ name = x; args = ys; body = e1 }, e2) ->
      pos, LetRec({ name = x; args = ys; body = g env e1 }, g env e2)
  | LetTuple(xts, y, e) when memt y env ->
      List.fold_left2
        (fun e' xt z -> pos, Let(xt, (pos, Var(z)), e'))
        (g env e)
        xts
        (findt y env)
  | LetTuple(xts, y, e) -> pos, LetTuple(xts, y, g env e)
  | e -> (pos, e)
